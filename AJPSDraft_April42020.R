...
bonica_ftm<-readRDS("FTM_BONICA_MERGE_AddedThru2016_V3.RDS")%>%
  ...
  mutate(ran.general=ifelse(cycle==2014,ran.general_2014, ran.general),
         winner=ifelse(cycle==2014, winner_2014, winner),
         party=ifelse(is.na(party)==TRUE, party_fixed, party),
         Incum.Chall=ifelse(cycle==2014, Incum.Chall_2014, Incum.Chall),
         Incum.Chall=ifelse(cycle==2016, Incum.Chall_2016, Incum.Chall),
...
## Use CleanFirstRun as marker of public financing.
## As financing status can vary throughout career,
## and we are using redistricting cycle observations of legislators,
## it makes the most sense to consider them as 'clean' if they started
## their career clean rather than if they were
...
## Subset to legislators and relevant district groups
all_np_party<-felm(Distance_NP~CleanFirstRun|UniqueDistrict_CensusGroup+party|0|UniqueDistrict_CensusGroup,
                   data=can_ideo, subset=CensusLines==1 & WonElection==1 )
summary(all_np_party)

## District Fixed Effects, Clean First Run (IV), Cluster SEs on District, Party
## Indicator Subset to legislators and relevant district groups, and to
## legislators who also have DistanceCFDyn
all_np_hasDistCF<-felm(Distance_NP~CleanFirstRun|UniqueDistrict_CensusGroup+party|0|UniqueDistrict_CensusGroup,
                       data=can_ideo, subset=CensusLines==1 & WonElection==1 & HasDistanceCFDyn==1 )
summary(all_np_hasDistCF)

## Create Latex Table
stargazer(all_np,all_np_party,all_np_hasDistCF,
          model.names = FALSE, model.numbers = TRUE,
          dep.var.labels.include = FALSE,
          dep.var.labels = c("NP Score Distance"),
          ...
          add.lines = list(c("District Fixed Effects", "\\checkmark","\\checkmark","\\checkmark"),
                           c("Party Fixed Effects","", "\\checkmark","\\checkmark"),
                           c("Has Dynamic CFScore","", "", "\\checkmark")),
          multicolumn = FALSE,
...
## Count Percentage of
## clean candidates who switched status at any point in the dataset
look<-can_fe%>%filter(CensusLines==1)
...
stargazer(modDyn, modNonDyn, 
          ...
          model.names = FALSE, model.numbers = FALSE,
          dep.var.labels.include = FALSE,
          #dep.var.labels = c("Non-Dynamic CFScore", "Dynamic CFScore", "Non-Dynamic CFScore", "Dynamic CFScore"),
          ...
          style = 'apsr',
          table.placement = "H",
          title=c("Paired T-Test Estimates: Arizona State Legislative Candidate Ideological Proximity to District, 2000-2012"),
...
####SI-A3 Interaction Effects Analysis####
...
####SI-A7 Clean From First Run Analysis####
